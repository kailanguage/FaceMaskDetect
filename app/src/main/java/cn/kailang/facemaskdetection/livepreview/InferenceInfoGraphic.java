...
public class InferenceInfoGraphic extends GraphicOverlay.Graphic {
    ...
    public synchronized void draw(Canvas canvas) {
        ...
        canvas.drawText(
                "original size: " + overlay.getImageWidth() + "x" + overlay.getImageHeight(),
                x,
                y,
                textPaint);
        canvas.drawText(
                "crop size: " + TF_API_INPUT_SIZE + "x" + TF_API_INPUT_SIZE,
                x,
                y+TEXT_SIZE,
                textPaint);
        ...
        if (framesPerSecond != null) {
            canvas.drawText(
                    "FPS: " + framesPerSecond + ", latency: " + latency + " ms", x, y + 2*TEXT_SIZE, textPaint);
        } else {
            canvas.drawText("Latency: " + latency + " ms", x, y + 2*TEXT_SIZE, textPaint);
        }
    }
}
